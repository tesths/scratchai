...
1. 先启动伴随程序。
2. 由伴随程序自动识别 Scratch 路径。
3. 如果没识别到，手动选择 `Scratch.exe` 或 `Scratch 3.exe`。
4. 从伴随程序里点击 `打开 Scratch`。
5. 连接成功后查看：
   - `当前角色`
   - `当前角色程序`

截至 2026-04-28，以上主流程已在本地真实 Windows 环境验证通过。
...
- 单机或小规模部署可以优先使用 `../ScratchDesktopCompanion-setup.exe`
...
5. 如果没有识别到，点击 `选择 Scratch`，手动选中真实 exe。
6. 点击 `打开 Scratch`。
...
1. 从仓库根目录取用 `ScratchDesktopCompanion-setup.exe`，或先将它复制到目标机器。
...
- 先让伴随程序完成路径识别或路径选择
- 再从伴随程序里点击 `打开 Scratch`

老师：

- 需要看状态时，点击托盘图标打开伴随程序窗口
- 如果首次没有路径，先执行 `选择 Scratch`
- 如果状态长时间不更新，点击 `重新连接`
- 关闭窗口不会退出程序，只会缩回托盘

## 8. 故障排查

### 现象 1：一直停在 `请先选择 Scratch 路径`
...
### 现象 2：一直停在 `请从伴随程序启动 Scratch Desktop`
...
- 确认是从伴随程序点击 `打开 Scratch`
- 不要让用户先手工双击 Scratch

### 现象 3：点了 `打开 Scratch` 但没有连上
