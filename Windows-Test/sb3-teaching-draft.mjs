...
import { validateBrief } from "./deepseek-workflow/core.mjs";
import { ensureDir, writeJson, writeText } from "./deepseek-workflow/file-system.mjs";
...
export function buildProjectSummary(project, options = {}) {
  ...
  const sourceFileName = sourceFilePath ? path.basename(sourceFilePath) : "";
...
function buildReadme({ outputDir, summary, runWorkflow, workflowOutputDir }) {
  ...
  if (runWorkflow) {
    ...
    lines.push(
      ...
      "手动运行示例：",
      "```powershell",
      `node Windows-Test\\run-deepseek-teaching-workflow.mjs --brief=\"${briefPath}\"`,
      "```"
    );
  }
...
export async function generateTeachingDraftFromSb3({
  ...
  if (runWorkflow) {
    const { runTeachingWorkflow } = await import("./deepseek-workflow/workflow.mjs");
