...
import {summarizeProgramAreaModulesFromProject} from '../packages/shared/src/index.js';
...
function isScratchExecutable(filePath) {
    const basename = path.win32.basename(filePath).trim().toLowerCase();
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                results.push({
                    ...
                    isScratchExecutable: resolved.target ? isScratchExecutable(resolved.target) : false
                });
            } catch (error) {
                results.push({
                    shortcutPath,
...
function pickScratchPageTarget(targets) {
    ...
    return inspectableTargets.find(isScratchPageTarget) ?? inspectableTargets[0] ?? null;
}
...
async function main() {
    ...
    if (!isScratchExecutable(scratchExecutablePath)) {
