...
const artifactDir =
    ...
    path.join(workspaceRoot, 'Windows-Test', 'artifacts', 'deepseek-live-seq-latest');
const appDataDir =
    argv.get('--appdata-dir') ??
    path.join(workspaceRoot, 'Windows-Test', 'tmp-live-deepseek-seq-appdata');
...
function listWindowsProcessesByNames(names) {
    ...
    if (result.status !== 0) {
        throw new Error(result.stderr.trim() || 'Failed to query Windows process list.');
    }
...
function sanitizeProcessInfo(processInfo) {
    return processInfo ? {
        ...
        debugPort: processInfo.debugPort ?? parseRemoteDebuggingPort(processInfo.CommandLine)
    } : null;
}
...
async function main() {
    ...
    const existingProcesses = listWindowsProcessesByNames([
        'Scratch 3.exe',
        'Scratch.exe',
    ...
    assert(
        ...
        `Please close existing Scratch or desktop companion processes first: ${JSON.stringify(existingProcesses)}`
    );
    ...
    await removeDirectoryWithRetries(appDataDir);
    ...
    try {
        ...
        launchedScratchProcess = await waitFor(async () => {
            ...
            const processDebugPort = parseRemoteDebuggingPort(candidate.CommandLine);
            return processDebugPort ? {...candidate, debugPort: processDebugPort} : null;
        }, {
            timeoutMs,
            intervalMs: 700,
        ...
        const removedAppData = await removeDirectoryWithRetries(appDataDir, {
            suppressFinalError: true
        });
        if (!removedAppData) {
            console.warn(
                `[verify-deepseek-live-seq] Failed to remove temp app data directory after retries: ${appDataDir}`
            );
        }
    }
}
