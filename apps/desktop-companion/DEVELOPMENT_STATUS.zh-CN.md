...
- `Windows-Test/verify-desktop-companion-ui.mjs`
  - 已补齐按平台解析 Electron 可执行文件
  - 当前可在 macOS 上直接跑源码版和打包版 UI 冒烟
- `Windows-Test/verify-desktop-companion-project-url-ui.mjs`
...
- `scripts/package-mac.mjs`
  - macOS `.app` / `.dmg` 打包
- `../../Windows-Test/electron-paths.mjs`
...
3. 如果改动了平台行为，优先看 `src/platform-adapter.ts`、`test/platform-adapter.test.mjs` 和 `test/package-mac.test.mjs`。
4. 如果改动了界面 DOM 或状态文案，必须同步更新 `verify-desktop-companion-ui.mjs`、`verify-desktop-companion-project-url-ui.mjs` 和 `verify-desktop-companion-real-e2e.mjs`。
